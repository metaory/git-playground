...
// import { catchTemplateMissmatch } from './catch.js'zzz
...
Object.defineProperty(global, 'cfg', {
  get: () => _config,
  set: (val) => {
    const parsed = JSON.parse(pupa(JSON.stringify(val), process.env))
    info(JSON.stringify(parsed, null, 2), C.yellow(val.version))
    _config = parsed
  },
  configurable: true
})

global.cfg = await new Config('mxcolr').init()
